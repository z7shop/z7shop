...
export default function EditProductPage() {
  ...
  const [form, setForm] = useState({
    ...
    sizes: '', colors: '', is_featured: false, is_new: false,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      toast.success(locale === 'fa' ? 'محصول به‌روزرسانی شد' : 'Product updated');
  ...
  return (
    <AdminLayout>
      <h1 className="text-2xl font-bold mb-6">{dict.adminPanel.editProduct}</h1>
      <form onSubmit={handleSubmit} className="card p-6 max-w-2xl space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="text-sm font-medium mb-1 block">{dict.product.price}</label>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-sm font-medium mb-1 block">{locale === 'fa' ? 'سایزها' : 'Sizes'}</label>
            <input className="input-field" value={form.sizes} onChange={e => setForm({ ...form, sizes: e.target.value })} dir="ltr" />
          </div>
          <div>
            <label className="text-sm font-medium mb-1 block">{locale === 'fa' ? 'رنگ‌ها' : 'Colors'}</label>
